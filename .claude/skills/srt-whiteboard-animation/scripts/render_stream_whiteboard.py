...
class FFmpegPipeWriter:
    """cv2.VideoWriter 的替身：BGR 帧直接喂进 ffmpeg，一步写成 H.264。

    OpenCV 的 mp4v 编码器在竖屏大尺寸（如 1080x1920）下会写出损坏的码流
    （解码报 "illegal dc vlc / header damaged"，后半段帧丢失），转码后成片会画到
    一半就停住。走管道就不再产生中间的 mp4v 文件，也不需要二次转码。
    """
    ...
    def isOpened(self) -> bool:  # noqa: N802 - 对齐 cv2.VideoWriter 接口
    ...
    def release(self) -> None:
        ...
        code = self.proc.wait()
        if code != 0:
            raise RuntimeError(f"ffmpeg 写入失败(exit {code}): {err}")
        print(f"  已写入 {self.frames} 帧 (H.264 直出)")
...
class RegionStreamRenderer:
    ...
    def __init__(self, image_bgr: np.ndarray, annotation: dict, cfg: sr.Config,
        ...
        # 全局两遍模式下起笔与上色不再相邻，先存下每个区域的笔迹中心点
        self._centers_cache: list[list] = []
    ...
    # ── 单个区域的起笔（ink）：铺线稿，返回笔迹中心点供 brush 上色复用 ──
    def _ink_pass(self, writer, frames: int, allowed) -> list:
        cfg = self.cfg
        if cfg.ink_path_mode == "skeleton":
            ...
            if strokes:
                ...
                self._lay_ink(writer, frames, samples, pen_lifts, allowed)
                centers = samples
            else:
            ...
            if path:
                ...
                self._lay_ink_grid(writer, frames, samples, pen_lifts, sample_cell, path, allowed)
                centers = [self._cell_center(c) for c in path]
            else:
                self._lay_ink(writer, frames, [], set(), None, allowed)
                centers = []
        self._centers_cache.append(centers)
        return centers

    # ── 主渲染 ──
    def render_to(self, raw_path: Path, total_ms: int, pass_order: str = "region") -> Path:
        cfg = self.cfg
        elements = sorted(self.ann["elements"], key=lambda e: e["reveal"]["startMs"])
        if FFMPEG is not None:
            writer = FFmpegPipeWriter(raw_path, cfg.fps, self.out_w, self.out_h)
        else:
        ...
        allowed_masks = [self._allowed_mask(el, elements[i + 1:]) for i, el in enumerate(elements)]

        try:
            if pass_order == "global":
                # 全局两遍：先把整张画的线稿按顺序起完，再整体上色
                total_dur = sum(el["reveal"]["durationMs"] for el in elements) or 1
                ink_total = total_dur * cfg.ink_weight / weight_sum
                color_total = total_dur * cfg.color_weight / weight_sum
                fill_static(elements[0]["reveal"]["startMs"])

                for idx, element in enumerate(elements):
                    share = element["reveal"]["durationMs"] / total_dur
                    frames = max(1, round(ink_total * share * cfg.fps / 1000))
                    self._ink_pass(writer, frames, allowed_masks[idx])
                    cur_ms += frames * ms_per_frame

                for idx, element in enumerate(elements):
                    share = element["reveal"]["durationMs"] / total_dur
                    frames = max(1, round(color_total * share * cfg.fps / 1000))
                    if cfg.color_fill == "contour-wipe":
                        self._wash_contour(writer, frames, allowed_masks[idx])
                    else:
                        self._wash_brush(writer, frames, self._centers_cache[idx], allowed_masks[idx])
                    cur_ms += frames * ms_per_frame

                gaze_until = max(total_ms, cur_ms + 500)
                self.drawn[...] = self.color_img.astype(np.float32)
                fill_static(gaze_until)
                return raw_path   # writer 由 finally 统一关闭
            ...
            for idx, element in enumerate(elements):
                ...
                centers = self._ink_pass(writer, ink_frames, allowed)
...
def _parse_args(argv=None):
    ...
    p.add_argument("--pass-order", default="region", choices=["region", "global"],
                   help="region=每个区域起笔后立刻上色（默认）；global=整张线稿全部起完再统一上色")
...
def main(argv=None) -> int:
    ...
    print(f"  区域数: {len(annotation['elements'])}, 总时长: {total_ms}ms, "
          f"笔迹: {cfg.ink_path_mode}, 上色: {cfg.color_fill}, 顺序: {args.pass_order}")

    if FFMPEG is not None:
        # 管道写入已经是 H.264，直接落到最终路径，不需要中间 mp4v 文件
        final = renderer.render_to(out_path, total_ms, args.pass_order)
    else:
        renderer.render_to(raw_path, total_ms, args.pass_order)
